...
with open("instrs.mac.vh", "w") as file:
    ...
    keys = ['oper', 'enc', 'aluop', 'alu1', 'alu2', 'wbsel', 'wb_en', 'cmpop', 'lsuop', 'type']
    ...
    lsuop = ["B", "H", "W", "BU", "HU", "X"]
    optype = ["COMMON", "BRANCH", "JUMP", "STORE", "LOAD"]
    ...
    for n, line in enumerate(lines):
        ...
        if d['type'] not in optype:
            print(f"{n}: Wrong type format: {d['type']}")
            fail = True
        out += f"`OPTYPE_{d['type']}, "
